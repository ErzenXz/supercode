...
import { shouldIndexFile, getFileExtension, getLanguageFromExtension, chunkCode, extractCodeSymbols, formatFileSize } from './utils'
...
export async function indexProject(projectId: string, onProgress?: (progress: IndexingProgress) => void) {
  try {
    ...
    const indexableFiles = files.filter(file => shouldIndexFile(file.path))
...
async function scanDirectory(dirPath: string): Promise<Array<{
  path: string
  relativePath: string
  name: string
  size: number
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const skipDirs = ['node_modules', '.git', 'dist', 'build', '.next', 'coverage', 'vendor', '__pycache__', 'target', 'bin', 'obj']
...
async function indexFile(projectId: string, file: {
  path: string
  relativePath: string
  name: string
  size: number
